...
const KitchenDashboard = ({navigation}: any) => {
  ...
  const printKOT = async (order: any) => {
    try {
      ...
      // Get Waiter Name (from the nested structure in your NestJS backend)
      const waiterName =
        order?.waiter?.name ||
        order?.items?.[0]?.order?.waiter?.name ||
        'Staff';

      let ticket = '';

      // Header
      ticket += `<C>KOT: ORDER #${oNumber}</C>\n`;
      ticket += divider;

      // Info Section (Date, Time, Waiter)
  ...
  useEffect(() => {
    ...
    const handleNewOrder = async (data: any) => {
      // 1. Transform data for the printer
      const tableNum = data?.table?.number || 'N/A';
      const orderNo = data?.orderNumber ? `#${data.orderNumber}` : '';

      if (data?.displayItems?.length) {
        const formattedOrder = {
          // id: data.id,
          tableNumber: tableNum,
          createdAt: data.createdAt,
          items: data.displayItems.map((item: any) => ({
            id: item.id,
            ...
            menuItem: {
              ...
              requiresPreparation: item?.menuItem?.requiresPreparation,
            },
            order: {
        ...
        // 2. Add the new items to the local state FIRST
        // This ensures the card appears on screen immediately
        setRawItems(prev => [...prev, ...formattedOrder.items]);

        // 3. Trigger Auto-print
        // This will now find 'PENDING' items and update them to 'PREPARING'
        handlePrintKOT(formattedOrder);
      }
    ...
    //   // 🔥 FIX: Transform data before printing
    //   if (data?.displayItems?.length) {
    //     const formattedOrder = {
    //       id: data.id,
    //       tableNumber: tableNum,
    ...
    //         id: item.id,
    //         quantity: item.quantity,
    //         status: 'PENDING', // socket items are always new
    //         isSpicy: item.isSpicy,
    //         menuItem: {
    //           name: item.menuItem?.name,
    //           requiresPreparation: item?.menuItem?.requiresPreparation,
  ...
  const handlePrintKOT = async (order: any) => {
    try {
      const itemsToPrint = order.items.filter(
        (item: any) =>
          item.status === 'PENDING' && item.menuItem?.requiresPreparation,
      );
      console.log(
        'IDs to update:',
        itemsToPrint.map((i: any) => i.id),
      );

      if (itemsToPrint.length === 0) return;

      // 1. Physical Print First
      await printKOT({...order, items: itemsToPrint});

      // 2. Local UI Update (Instant feedback for user)
      const idsToUpdate = new Set(itemsToPrint.map((item: any) => item.id));
      setRawItems(prev =>
        prev.map((item: any) =>
          idsToUpdate.has(item.id) ? {...item, status: 'PREPARING'} : item,
        ),
      );

      // 3. Sequential API Updates (Prevents 500 errors from race conditions)
      for (const item of itemsToPrint) {
        try {
          console.log(`Updating item ${item.id} to PREPARING...`);
          await api.patch(`/orders/item/${item.id}/status`, {
            status: 'PREPARING',
          });
        } catch (itemErr) {
          // Log the specific error for this specific item
          console.log('itemErr', itemErr);
        }
      }

      Toast.show({type: 'success', text1: 'KOT Printed & Status Updated'});

      // 4. Final Sync
      setTimeout(() => fetchKitchenQueue(), 1000);
    } catch (err) {
      console.error('Print process failed:', err);
      Toast.show({
        type: 'error',
        text1: 'Printer Error',
        text2: 'Status not updated',
      });
    } finally {
      setPrintingId(null);
    }
  ...
  //   try {
  //     const itemsToPrint = order.items.filter(
  //       item => item.status === 'PENDING' && item.menuItem?.requiresPreparation,
  //     );
  ...
  const renderOrderTicket = ({item: order}: {item: any}) => {
    const hasPendingItems = order.items.some(
      (i: any) => i.status === 'PENDING' && i.menuItem?.requiresPreparation,
    );
      ...
      <View
        ...
        <ScrollView
          ...
          <View style={styles.itemsList}>
            {(order.displayItems || order.items).map(
              (item: any) => (
                console.log('ORDER ITEMS:', order.items),
                console.log('DISPLAY ITEMS:', order.displayItems),
                (
                  <View
                    key={item.id}
                    style={[
